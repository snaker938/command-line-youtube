...
class CommandParser:
    ...
    def execute_command(self, command: Sequence[str]):
        ...
        elif command[0].upper() ==  "CREATE_VIDEO":
            if len(command) != 4:
                raise CommandException(
                    "Please enter CREATE_VIDEO command followed by a video_title, a video_id and video_tags. ( with tags in the form: example1,example2,example3  and so on. And with the title separated with '-': example-video-cats    and with the id in the form: example_video_id     if there are no tags, please enter '[]' for video_tags)")
    ...
    def _get_help(self):
        ...
        Available commands:
            ...
            CREATE_VIDEO <video_title> <video_id> <video_tags> - Creates a new video on the platform.
            DELETE_VIDEO <video_id> - Deletes the selected video from the platform
            HELP - Displays help.
